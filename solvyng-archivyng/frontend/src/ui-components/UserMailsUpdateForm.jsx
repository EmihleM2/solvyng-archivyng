...
import { getUserMails } from "../graphql/queries";
import { updateUserMails } from "../graphql/mutations";
const client = generateClient();
export default function UserMailsUpdateForm(props) {
  const {
    id: idProp,
    userMails: userMailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userMailsRecord
      ? { ...initialValues, ...userMailsRecord }
      : initialValues;
    setUser_email(cleanValues.user_email);
    setMail_subject(cleanValues.mail_subject);
    setMail_message(cleanValues.mail_message);
    setErrors({});
  };
  const [userMailsRecord, setUserMailsRecord] =
    React.useState(userMailsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserMails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserMails
        : userMailsModelProp;
      setUserMailsRecord(record);
    };
    queryData();
  }, [idProp, userMailsModelProp]);
  React.useEffect(resetStateValues, [userMailsRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserMails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userMailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserMailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="User email"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userMailsModelProp)}
