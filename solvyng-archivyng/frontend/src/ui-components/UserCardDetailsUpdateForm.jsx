...
import { getUserCardDetails } from "../../queries";
import { updateUserCardDetails } from "../../mutations";
const client = generateClient();
export default function UserCardDetailsUpdateForm(props) {
  const {
    id: idProp,
    userCardDetails: userCardDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userCardDetailsRecord
      ? { ...initialValues, ...userCardDetailsRecord }
      : initialValues;
    setCard_name(cleanValues.card_name);
  ...
  const [userCardDetailsRecord, setUserCardDetailsRecord] = React.useState(
    userCardDetailsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getUserCardDetails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserCardDetails
        : userCardDetailsModelProp;
      setUserCardDetailsRecord(record);
    };
    queryData();
  }, [idProp, userCardDetailsModelProp]);
  React.useEffect(resetStateValues, [userCardDetailsRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserCardDetails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userCardDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserCardDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Card name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userCardDetailsModelProp)}
